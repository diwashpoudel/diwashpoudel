          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">
                <a href="{{ route('category.index') }}">Category Listing</a></li>
                <li class="breadcrumb-item active">
                   Category Listing</li>
              ...
              <div class="card-body p-0">
                <div class="d-md-flex">
                  <div class="p-3 flex-fill" style="overflow: hidden">
                    @if(isset($detail))
                    {{Form::open(['url'=>route('category.update',$detail->id) , 'class'=>'form ','files'=>true ])}}
                    @method('patch')
                    @else
                    ...
                    <div class="form-group row">
                       ...
                       <div class="col-sm-9">
                         {{ Form::text('title',@$detail->title,['class'=>'form-control form-control-sm','required'=>'true','placeholder'=>'Enter Your Category Title Here' ,'id'=>'title']) }}
                        @error('title')
                ...
                <div class="form-group row">
                       ...
                       {{  Form::label('parent_id', 'Parent:',['class'=>'col-sm-3']) }}
                    <div class="col-sm-9">
                         {{ Form::select('parent_id',@$parent_cats,@$detail->parent_id,['class'=>'form-control form-control-sm','required'=>'false' ,'placeholder'=>'---- Select Any One -----' ,'id'=>'parent_id']) }}
                        @error('parent')
                        <span class="text-danger">{{ $message }}</span>
               ...
               <div class="col-sm-9">
                    {{ Form::select('brand_id[]',@$brand_data,@$detail->brandIds->pluck('brand_id'),['class'=>'form-control ','required'=>'false' ,'multiple'=>'false','id'=>'brand_id']) }}
                   {{-- @error('brand')
                   <span class="text-danger">{{ $message }}</span>
              ...
              <div class="col-sm-3">
                  ...
                  <div class="col-sm-2">
                    @if(isset($detail) && $detail->image !== null)
                      @php $url = asset('uploads/category/'.$detail->image);@endphp
                      @else
